...
if (canvas.getContext) {
  ...
  for (let i = 0; i < game.imgsrc.length; i++) {
  ...
  const cat = {
    ...
    update: function () {
      ...
      let n = Math.floor(this.imgPosition / 4);

      context.drawImage(
        this.img,
        0,
        n * this.height + n, // + n pixels for gap between sprites to prevent border artifact
        this.width,
        this.height,
        this.x,
        this.y,
  ...
  trailElement.prototype.update = function (opacity) {
    ...
    let background = context.createLinearGradient(
      this.x + this.width / 2,
      this.y,
      this.x + this.width / 2,
      this.y + this.height
    ...
    background.addColorStop(0.83, "rgba(0, 153, 253, " + opacity + ")");
    background.addColorStop(0.831, "rgba(102, 51, 253, " + opacity + ")");
    background.addColorStop(1.0, "rgba(102, 51, 253, " + opacity + ")");
  ...
  let block = function (x, y, img, isUpper, height) {
    this.x = x;
    this.y = y;
    this.width = 100;
  ...
  block.prototype.draw = function () {
    ...
    context.drawImage(
      this.img,
      0,
      this.isUpper ? 400 - this.height : 0,
      this.width,
      this.height,
      this.x,
      this.y,
  ...
  let backgroundElement = function (x, y, speed) {
    this.x = x;
    this.y = y;
    this.width = 19;
    this.height = 19;
  ...
  backgroundElement.prototype.update = function () {
    let n = Math.floor(this.imgPosition / 7);

    context.drawImage(this.img, n * this.width, 0, this.width, this.height, this.x, this.y, this.width, this.height);
  ...
  for (let i = 0; i < 10; i++) {
    backgroundElements.push(
      new backgroundElement(Math.random() * (game.width - 50) + 50, (i + 1) * 60, Math.random() * 4 + 2)
    );
  }

  let menuButton = function (x, y, width, height, text, img) {
    this.x = x;
    this.y = y;
    this.width = width;
    this.height = height;
  ...
  let playButton = new menuButton(game.width / 2 - 98, 270, 200, 60, "Play");
  ...
  function start() {
    ...
    for (let i = 0, j = 0; i < 4; i++) {
      if (i % 2 == 0) {
        ...
        let blockGap = Math.random() * 100 + 200;
        game.blocks.push(
          new block(
            game.width + 150 + j++ * 700,
            game.blocks[i - 1].height + blockGap,
            game.images[9],
            false,
            game.height - game.blocks[i - 1].height - blockGap
          )
        );
      }
    }
  ...
  function menuSelect(event) {
    ...
    for (let i = 0; i < skinButtons.length; i++) {
      if (
        collisionPointObj(event.pageX - canvas.offsetLeft - 10, event.pageY - canvas.offsetTop - 10, skinButtons[i])
      ) {
        cat.skin = i;
    ...
    for (let i = 0; i < skinButtons.length; i++) {
      ...
      if (cat.skin === i) {
        context.fillRect(
          skinButtons[i].x + 2,
          skinButtons[i].y + 2,
          skinButtons[i].width - 3,
          skinButtons[i].height - 3
        );
      }
  ...
  function update() {
    ...
    // background
    let background = context.createRadialGradient(
      game.width / 2,
      game.height / 2,
      0,
      game.width / 2,
